...
const JS_WORKER_QUEUE_CAPACITY: usize = 4_096;
...
impl V8RuntimeInner {
    ...
    async fn make_actor(
        ...
        let lane_queue = JsWorkerQueue::new(
            JS_WORKER_QUEUE_CAPACITY,
            Some(
                WORKER_METRICS
...
impl JsModule {
    ...
    pub async fn create_instance(&self) -> JsInstance {
        ...
        // Pooled instances are checked out to one caller at a time, so keep the
        // private per-instance queue as a rendezvous channel. Unlike the shared
        // instance lane, there is no replay path for buffered pooled requests
        // across worker replacement.
        let request_queue = JsWorkerQueue::new(0, None);
...
impl JsWorkerQueue {
    fn new(capacity: usize, queued_requests_metric: Option<IntGauge>) -> Arc<Self> {
        let (tx, rx) = flume::bounded(capacity);
        let queue = Arc::new(Self {
            tx,
            rx,
            len: queued_requests_metric,
        });
        if let Some(metric) = &queue.len {
            metric.set(0);
        }
        queue
    }
...
impl JsInstance {
    ...
    async fn send_request<T>(
        ...
        if trapped {
            self.worker_state.mark_trapped();
        }
        Ok(value)
    }
...
async fn spawn_instance_worker(
    ...
    let worker_queue_metric = request_queue.len.clone();
